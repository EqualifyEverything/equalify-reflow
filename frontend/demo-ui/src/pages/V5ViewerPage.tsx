...
import { MarkdownViewer } from '@/components/v5/MarkdownViewer';
import { AgentDetailModal } from '@/components/v5/AgentDetailModal';
import { DecisionList } from '@/components/v5/DecisionList';
import { JobNavigationPanel } from '@/components/v5/JobNavigationPanel';
import { useV5Stream } from '@/hooks/useV5Stream';
import type { AgentThinkingData } from '@/types/v5-events';
import {
  Play,
  Square,
  RefreshCw,
...
const API_KEY_STORAGE_KEY = 'v5-viewer-api-key';

export function V5ViewerPage() {
  ...
  const [useOptimized, setUseOptimized] = useState(true); // Default to optimized pipeline
  ...
  const handleFileUpload = useCallback(
    async (file: File) => {
      ...
      try {
        ...
        // Add optimized query parameter
        const url = new URL(`${API_URL}/api/v5/process`);
        url.searchParams.set('optimized', useOptimized.toString());
    ...
    [apiKey, clearEvents, connect, setSearchParams, useOptimized]
  );
      ...
      <header className="bg-uic-blue text-white px-6 py-4 shadow-lg">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h1 className="text-xl font-bold">V5 Pipeline Viewer</h1>
