...
class TestMergeSectionAttributes:
    """Test the _merge_section_attributes static method."""

    def test_single_page_attributes(self):
        """Single page section should return that page's attributes."""
        from src.services.pipeline_viewer_models import SectionEntry

        section = SectionEntry(
            index=0, heading_text="Intro", heading_level=2,
            pages=[1], markdown="", outline_position=0,
        )
        structure = StructureResult(
            page_attributes={
                1: PageAttributes(
        ...
        merged = PipelineViewerService._merge_section_attributes(section, structure)

        assert merged is not None
        assert merged.layout == LayoutType.DOUBLE_COLUMN
        assert merged.is_academic is True
        assert merged.has_tables is True
        assert merged.has_images is False

    def test_multi_page_or_flags(self):
        """Boolean flags should be OR'd across pages."""
        from src.services.pipeline_viewer_models import SectionEntry

        section = SectionEntry(
            index=0, heading_text="Intro", heading_level=2,
            pages=[1, 2], markdown="", outline_position=0,
        )
        structure = StructureResult(
            page_attributes={
                1: PageAttributes(layout=LayoutType.DOUBLE_COLUMN, has_tables=True),
        ...
        assert merged is not None
        assert merged.has_tables is True  # from page 1
        assert merged.has_images is True  # from page 2

    def test_no_matching_pages(self):
        """Should return None when no page attributes match."""
        from src.services.pipeline_viewer_models import SectionEntry

        section = SectionEntry(
            index=0, heading_text="Intro", heading_level=2,
            pages=[5], markdown="", outline_position=0,
        )
        structure = StructureResult(
            page_attributes={
