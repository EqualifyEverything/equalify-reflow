"""Unit tests for ProcessingWorker shutdown requeueing.
...
@pytest.fixture
def mock_services():
    """Create mock services for ProcessingWorker."""
    storage = MagicMock()
    queue = AsyncMock()
    job = AsyncMock()
...
@pytest.mark.asyncio
async def test_processing_worker_requeues_on_shutdown(mock_services):
    ...
    assert isinstance(requeued_payload, ProcessingQueuePayload)
